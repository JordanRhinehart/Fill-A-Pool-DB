...
<!-------------------------------------------------------------------------------------------------->
          ...
          <form class="formy" action="TruckFillingPageQuery.php" method="POST">
              ...
              <div class="table-form c1">
                    <table border="1" cellspacing="5" cellpadding="5" width="100%">
                        ...
                        <?php
                            require_once('connection.php');
                            $result = mysqli_query($conn,"SELECT * FROM truck_fill_locations WHERE name = 'null' ORDER BY location_id ASC");
                            for($i=0; $row = mysqli_fetch_array($result); $i++){
                        ?>
                            <tr>
                                <td><label><?php echo $row['location_id'] /* ?? 'Null'*/; ?></label></td>
