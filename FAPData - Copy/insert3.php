...
//if variables are not empty or invalid AND if the proper button was pressed
    ...
    // } else
    if(!empty($name) && !is_numeric($name) && !empty($address))
    {


        //save to database
        $query = "insert into truck_fill_locations (name, address, price_per_gallon, comments) 
        VALUES ('$name', '$address', '$price', '$comments')";
